...
class UniswapV2LogReader(ILogReader):
    ...
    def get_swapped_tokens(self, logs, is_eth_first_token):
        ...
        eth_token_amount = self.get_eth_token_amount(logs)
        for log in logs:
            ...
            if topics[0] == swap_method_hash_v2:
                ...
                if decoded_abi[0] == eth_token_amount or decoded_abi[2] == eth_token_amount:
                    if decoded_abi[0] != 0:
    ...
    def get_eth_token_amount(self, logs):
        for log in logs:
            if log[self.LOGS_ADDRESS_FIELD] == self.ETH_ADDRESS:
                return int(log[self.LOGS_DATA_FIELD], 16)
